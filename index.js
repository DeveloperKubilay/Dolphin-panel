...
app.use(bodyParser.urlencoded({
  limit: settings.posttojson.limit + 'mb',
  extended: true,
...
app.listen(Number(settings.port), () => { console.log("Panel Running at ", Number(settings.port)) })
...
io.use((socket, next) => {
  ...
  if (socket.handshake.auth.type === "panelusers" &&
    ...
    panelusersdb.push({ url: url, socket: socket.id, mail: socket.handshake.auth.mail }); return next();
  }//Users
      ...
      if (msg.action === "installos") {
        ...
        if (msg.changeos && osdb.get("os").filter(z => z.name === msg.changeos).length) {
          ...
          if (gillsdb.get("gills").filter((x) => x.ip === tmpdb.ip).length) {
            ...
            gillsdb.set(gillname + "-database", tempgill)
          } catch { }
    ...
    try {
      ...
      if (ddosdb.get("user-" + req.headers["x-forwarded-for"]) > settings.ddosprotect.limit) {
        ...
        if (settings.ddosprotect.removeban) {
          setTimeout(() => {
            ddosdb.del("blacklist-" + req.headers["x-forwarded-for"])
          }, settings.ddosprotect.removeban)
        }
      } else {
        ddosdb.add("user-" + req.headers["x-forwarded-for"], 1)
        if (!ddosdb.get("timeout-" + req.headers["x-forwarded-for"])) {
          ddosdb.set("timeout-" + req.headers["x-forwarded-for"], true)
          setTimeout(() => {
            ddosdb.del("user-" + req.headers["x-forwarded-for"])
            ddosdb.del("timeout-" + req.headers["x-forwarded-for"])
          }, settings.ddosprotect.time)
        }
      }
      return next();
    } catch { return next(); }
...
app.post('/api/users/remove/', function (req, res) {
  ...
  if (usersdb.get("servers-" + req.body.mail).length) {
    return res.send({
      message: "In order to delete User, you need to delete the servers here",
...
app.post("/api/gills/add", function (req, res) {
  ...
  if (req.body.edit) {//Edit
    ...
    oldmap.ports = oldmap.ports.forEach((x) => {
      if (!olddb.ports.filter(z => z.port === x.port).length) usingports.push(x)
    })
    req.body.gill.ports.map((x) => { 
      x.port = x.port.replaceAll("\n", "").trim()
      x.panelport = x.panelport.replaceAll("\n", "").trim()
      if (!usingports.filter(z => z.port === x.port).length) ports.push(x) 
      })
  ...
  if (!gillsdb.get(req.body.gill.name + "-database")) {
    gillsdb.set(req.body.gill.name + "-database", {
      ...
      "vnc": 0,
      "ports": req.body.gill.ports.map((x) => {
        x.port = x.port.replaceAll("\n", "").trim()
        x.panelport = x.panelport.replaceAll("\n", "").trim()
        return x
      })
...
app.post("/api/gills/remove/", function (req, res) {
  ...
  if (gillsdb.get("gills").filter(z => z.name === req.body.name).length) {
    ...
    serversdb.get("servers").map((x) => {
      if (serversdb.get(x + "-server").ip === tmpdb.ip) {
        servers.push(x)
      }
    })
  }
  if (servers.length) {
    return res.send({
      message: "In order to delete Gill, you need to delete the servers here",
...
app.post("/api/servers/add", function (req, res) {
  ...
  if (req.body.edit) {//edit
    ...
    if (!gillsdb.get("gills").filter((z) => z.ip === serversdb.get(req.body.edit + "-server").ip).length) return res.send({
      message: "Gill not found",
      color: "red", url: "/admin/servers"
    })
    if (req.body.code.id) {
      x.id = req.body.code.id
  ...
  if (req.body.edit) {
    return res.send({ message: "Edited to server", color: "green", url: "/admin/editservers/" + req.body.edit })
...
app.post("/api/servers/remove", function (req, res) {
  ...
  if (serversdb.get(req.body.machine + "-server") &&
    ...
    if (gillsdb.get("gills").filter(z => serversdb.get(req.body.machine + "-server").ip === z.ip)) {
      ...
      if (serverdata.ports.mainport) {
        if (serverdata.ports.mainport.split(":").length > 1) {
