$("#addserversform").submit(function (event) {
  ...
  try {
    ...
    if (ejs.disks) {//Disks
      ...
      disks.map((x) => {
        if (ejs.disks.filter((z) => z.name === x.name).length) {//Edit disk
    ...
    if (edit) {//Ports
      ...
      if (ejs.ports) {
        ejs.ports.map((x) => {
          ...
          else { ports.push({ deletedport: true, code: x }) }
        })//Delete Port
      }; var tmpport = ports; ports = [];
      tmpport.map((x) => {//New Port
        if (x.deletedport === true) {
          ports.push(x)
...
$("#deleteserver").click(function () {
  var posting = $.ajax({
    url: "/api/servers/remove/", type: "POST", data: {
